 ...
 *******************************************************************************/
/*!
 * @brief ADC sampling simulation function
 *
 * @param *u U-ADC output buffer/FFT real part output buffer (Ph2)
 * @param *i I-ADC output buffer/FFT real part output buffer (Ph1)
 * @param line Phase number to be filled
 */
...
void Metering_Init(void)
{
	...
	fcn_out = METERLIBFFT3PH_SetCalibCoeffPh1(&ui, 452, 13.75, &afe1_offs, 0.0, 0.0);
	fcn_out = METERLIBFFT3PH_SetCalibCoeffPh2(&ui, 452, 13.75, &afe2_offs, 0.0, 0.0);
	fcn_out = METERLIBFFT3PH_SetCalibCoeffPh3(&ui, 452, 13.75, &afe3_offs, 0.0, 0.0);
...
void Metering_Process(void)
{
	...
	//GPIO_Set (FGPIOF, PIN7);    /* code performance measurement start pulse */
	/* main calculation (FFT, scaling, averaging) */
	METERLIBFFT3PH_CalcMain(&ui);
	//GPIO_Clr (FGPIOF, PIN7);     /* code performance measurement stop pulse */
	...
	/* calculates active energy increment and treshold value for active energy timer */
	//tmr_cmp = TMRCMPVAL(METERLIBFFT3PH_CalcWattHours(&ui, &whi, &whe, frequency),TMRCLK);
	METERLIBFFT3PH_CalcWattHours(&ui, &whi, &whe, frequency);
	/* calculates reactive energy increment and treshold value for reactive energy timer */
	//tmr_cmp = TMRCMPVAL(METERLIBFFT3PH_CalcVarHours(&ui, &varhi, &varhe, frequency),TMRCLK);
	METERLIBFFT3PH_CalcVarHours(&ui, &varhi, &varhe, frequency);
	...
	{ /* simulate asynchronous reading of all averaged values */
		...
		read_period = 0;
		PRINTF("Update every 50th measurement\r\n");
		PRINTF("Total: Whi:%lu, Whe:%lu, VARhi:%lu, VARhe:%lu, freq:%lu mHz \r\n",whi,whe,varhi,varhe,frequency);
		PRINTF("Phase 1 data: URMS:%.1lf, IRMS:%.1lf, P:%.1lf, Q:%.1lf, S:%.1lf, PF:%.1lf, THDU:%.1lf, THDI:%.1lf\r\n",urms1,irms1,p1,q1,s1,pf1,thdu1,thdi1);
		PRINTF("Phase 2 data: URMS:%.1lf, IRMS:%.1lf, P:%.1lf, Q:%.1lf, S:%.1lf, PF:%.1lf, THDU:%.1lf, THDI:%.1lf\r\n",urms2,irms2,p2,q2,s2,pf2,thdu2,thdi2);
		PRINTF("Phase 3 data: URMS:%.1lf, IRMS:%.1lf, P:%.1lf, Q:%.1lf, S:%.1lf, PF:%.1lf, THDU:%.1lf, THDI:%.1lf\r\n",urms3,irms3,p3,q3,s3,pf3,thdu3,thdi3);
